...
- [ ] **`@fractality/fractality`'s CLI reads the wrong `package.json`** (found 2026-08-14, pre-existing — reproduces on `main`, unrelated to any dependency work). `packages/fractality/bin/fractality.js:13` does `readJsonSync('./package.json')`, a relative path resolved against `process.cwd()` rather than the CLI's own location, so it reads whatever `package.json` the user happens to be standing in and feeds it to `update-notifier`. Consequences: run from a directory whose `package.json` has no `name`/`version` (the repo root, for one) the CLI dies outright with `Error: pkg.name and pkg.version required`; run inside a real user project it "works" but checks npm for updates to _the user's_ package instead of `@fractality/fractality`. Should read its own manifest via `__dirname`, the same way `packages/mandelbrot/src/theme.js` does. Note this is the same class of bug as the completed "replace imports of `package.json` for better support" item above, which missed this call site
